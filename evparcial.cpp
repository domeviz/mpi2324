...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        ...
        {
            MPI_Send(datos.data() + i * (tam / nprocs), tam / nprocs, MPI_INT, i, 0, MPI_COMM_WORLD);
        }
        std::copy(datos.begin(), datos.begin() + tam / nprocs, datosp.begin());
    }
    else
    {
        MPI_Recv(datosp.data(), tam / nprocs, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
